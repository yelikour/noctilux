...
- Annotation prototype: internal schema, prototype COCO/YOLO readers, and bbox resize/flip geometry helpers in `noctilux.annotations`; not wired into `noctilux run`.
...
- v0.7.2: bbox sync primitives for resize / horizontal flip / vertical flip (internal only; annotation sync not yet supported in `noctilux run`)
...
- 尚未正式支持 detection / segmentation annotation 同步增强；v0.7.2 只增加内部 bbox resize/flip geometry helpers，未接入 `noctilux run`。
