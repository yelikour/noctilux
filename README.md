...
- Crop window metadata in transform log for all crop transforms (v0.9.0)
...
v0.8.0 adds opt-in COCO-like bbox-only annotation IO. v0.8.1 adds guardrails for resume, parallel, and output overwrite safety. It is experimental and is not full annotation sync. Image-only configs do not need an `annotations` section and keep the same default behavior.
...
- v0.7.1: annotation schema / parser prototype
...
- v0.8.1: annotation IO guardrails — reject resume/parallel, output overwrite safety, improved warning traceability
- v0.9.0: crop window metadata in transform log for all crop transforms
...
- Annotation IO 仍是实验性 opt-in；v0.9.0 只支持 COCO bbox-only minimal integration，不支持 mask / polygon / keypoint / rotate / crop annotation sync。crop_window metadata 已在 transform log 中暴露，但 annotation crop sync 尚未接入。
