...
- Annotation synchronization for detection/segmentation/keypoint tasks is still not formally supported. v0.7.2 adds internal bbox resize/flip geometry helpers on top of parser prototypes, but they are not wired into `noctilux run`; current functionality remains image-only augmentation.
