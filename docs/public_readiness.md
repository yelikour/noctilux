...
- Annotation synchronization for detection/segmentation/keypoint tasks is still not formally supported. v0.7.3 adds internal bbox crop clipping/filtering helpers on top of parser and resize/flip prototypes, but they are not wired into `noctilux run`; current functionality remains image-only augmentation.
